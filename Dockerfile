...
RUN ARCH=$(uname -m) && case $ARCH in aarch64) ARCH="arm64";; x86_64) ARCH="amd64";; esac && echo "ARCH: " $ARCH && \
    ...
    KUBECTL_NODE_MAINTAIN_VERSION=0.0.1 && wget https://github.com/futuretea/kubectl-node-maintain/releases/download/v${KUBECTL_NODE_MAINTAIN_VERSION}/kubectl-node-maintain_${KUBECTL_NODE_MAINTAIN_VERSION}_linux_${ARCH}.tar.gz && tar -xvf kubectl-node-maintain_${KUBECTL_NODE_MAINTAIN_VERSION}_linux_${ARCH}.tar.gz && chmod +x kubectl-node-maintain && mv kubectl-node-maintain /usr/bin/kubectl-node-maintain && \
